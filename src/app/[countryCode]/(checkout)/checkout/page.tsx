import { Metadata } from 'next'
import { notFound } from 'next/navigation'

import Wrapper from '@components/checkout/sub-components/payment-wrapper'
import CheckoutForm from '@components/checkout/templates/checkout-form'
import CheckoutSummary from '@components/checkout/templates/checkout-summary'
import { Container } from '@components/common/sub-components/container'
import { enrichLineItems, retrieveCart } from '@lib/data/cart'
import { getCustomer } from '@lib/data/customer'

export const metadata: Metadata = {
  title: 'Checkout',
}

const fetchCart = async () => {
  const cart = await retrieveCart()
  if (!cart) {
    return notFound()
  }

  if (cart?.items?.length) {
    const enrichedItems = await enrichLineItems(cart?.items, cart?.region_id)
    cart.items = enrichedItems
  }

  return cart
}

export default async function Checkout({
  searchParams,
}: {
  searchParams: { [key: string]: string | string[] | undefined }
}) {
  const cart = await fetchCart()
  const customer = await getCustomer()

  const searchParamsLoaded = await searchParams

  return (
    <Container className="mx-0 grid max-w-full grid-cols-1 gap-y-4 bg-secondary large:grid-cols-[1fr_416px] large:gap-x-10 2xl:gap-x-40">
      <Wrapper cart={cart}>
        <CheckoutForm cart={cart} customer={customer} />
        <CheckoutSummary cart={cart} searchParams={searchParamsLoaded} />
      </Wrapper>
    </Container>
  )
}
